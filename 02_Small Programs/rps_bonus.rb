...
def display_results(human, computer)
  if win?(human[:choice], computer[:choice])
    prompt("You won!")
  elsif win?(computer[:choice], human[:choice])
    prompt("Computer won!")
  else
    prompt("It's a tie!")
  end
...
def reset_scores(human, computer) # Scores are reset after the mini-tournament is over.
  human[:score] = 0
  computer[:score] = 0
end
